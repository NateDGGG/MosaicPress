  ...
  async function checkout() {
    ...
    try {
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ email, lines: lines.map((l) => ({ itemId: l.itemId, quantity: l.quantity })) }),
      });
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <label className="mb-1 block text-xs font-medium text-slate-500">Email (for receipt)</label>
        <input
          type="email"
          value={email}
