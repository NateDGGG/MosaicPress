...
// Topic (tag) taxonomy: many-to-many topics on items, with a configurable
// default topic that unassigned content falls under.
...
export async function deleteTag(id: string) {
  ...
  for (const l of links) {
    ...
    if (count <= 1) {
      await prisma.itemTag.create({ data: { itemId: l.itemId, tagId: def.id } }).catch(() => {});
    }
  }
